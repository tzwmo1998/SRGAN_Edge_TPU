...
class DIV2K:
    ...
    def dataset(self, batch_size=16, repeat_count=None, random_transform=True, drop_remainder=True):
        ds = tf.data.Dataset.zip((self.lr_dataset(), self.hr_dataset()))
        if random_transform:
            ds = ds.map(lambda lr, hr: random_crop(lr, hr, scale=self.scale), num_parallel_calls=AUTOTUNE)
            # ds = ds.map(random_rotate, num_parallel_calls=AUTOTUNE)
            # ds = ds.map(random_flip, num_parallel_calls=AUTOTUNE)
        # make sure batch size always constant (fixed graph)
...
def random_crop(lr_img, hr_img, hr_crop_size=800, scale=2):
    lr_crop_size = hr_crop_size // scale
    lr_img_shape = tf.shape(lr_img)[:2]
    lr_width = lr_img_shape[1]
    lr_height = lr_img_shape[0]
    if lr_width < lr_crop_size:
      lr_img = tf.tile(lr_img, [1,2,1])
      hr_img = tf.tile(hr_img, [1,2,1])
      print(lr_img.shape) 
    if lr_height < lr_crop_size:
      lr_img = tf.tile(lr_img, [2,1,1])
      hr_img = tf.tile(hr_img, [2,1,1])
      print(lr_img.shape)
    lr_img_shape = tf.shape(lr_img)[:2]
